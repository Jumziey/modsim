...
void pos_from_brown(int n,Vec* L, double noise, double alpha, double deltat, Vec* pos, Vec* force)
{
  int i;
  for(i=0; i<n; i++) {
    pos[i].x = inrange(pos[i].x + force[i].x*deltat/alpha + noise*dran_sign()*sqrt(3)*deltat,L->x);
    pos[i].y = inrange(pos[i].y + force[i].y*deltat/alpha + noise*dran_sign()*sqrt(3)*deltat,L->y);
  }
}

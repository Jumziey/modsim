...
void pos_from_brown(int n, double noise, double alpha, double deltat, Vec* pos, Vec* force)
{
  int i;
  
  for(i=0; i<n; i++) {
    pos[i].x = pos[i].x + (force[i].x/alpha + noise*dran_sign()*sqrt(3))*deltat;
    pos[i].y = pos[i].y + (force[i].y/alpha + noise*dran_sign()*sqrt(3))*deltat;
  }
}
...
void measure(int n, Vec *L, Vec *pos, double *epot, double *ekin)
{
  int i, j;
  Vec dist;

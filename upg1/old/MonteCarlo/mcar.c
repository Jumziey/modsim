...
int step(Par *par, Vec *pos, Vec *vel, Vec *force)
{
  ...
  if (par->alpha > 0.0)
    pos_from_monte_carlo(par->n,par->step, &par->L, par->t, par->noise, par->alpha, par->deltat, pos, force);
  else {
    printf("BREAKAGE!!!!! AAAH ALPHA CANT BE ZERO FFS!!!!\n");
    exit(1);
  }
  return 0;
}
